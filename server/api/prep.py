...
class Prep(Resource):
    ...
    def toAnswer(self, init_ngram, pi, corrections): # 將結果組合回原本的片段
        res = [] 
        for corr, count in corrections:
            temp = init_ngram[:]
            prep = [ (word, pos) for word, pos in corr if pos == 'PREP']
            if 'PREP' in [pos for _, pos in init_ngram]:
                if prep != []:
                    temp[pi] = prep[0]
                else:
                    temp.remove(temp[pi])
                ...
                if prep != []:
                    temp.insert(pi, prep[0])
            res.append((' '.join(word for word, _ in temp), count))
        return res
            ...
            for ngram, idx in res_RTorUT: # [[('discussed', 'V'), ('about', 'prep'), ('it', 'PRP')], 1]
                ...
                if corrections:
                    ...
                    if any( self.isModified(ngram, corr, idx) for corr, _ in corrections ):
                        results += self.toAnswer(ngram[:], idx, corrections)
                ...
                for i in idx: # idx => set
                    ...
                    if corrections:
                        ...
                        if any( self.isModified(ngram[:], corr, i) for corr, _ in corrections ):
                            results += self.toAnswer(ngram[:], i, corrections)
